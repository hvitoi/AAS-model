  ...
  const renderList = () => {
    if (aasList.length !== 0) {
      return aasList.map((aas) => (
        <tr key={aas._id}>
          <td>
            <Link to={"/aas/" + aas._id}>{aas._id}</Link>
          </td>
          <td>{aas.namePlate}</td>
    ...
    return (
      <tr>
        <td colSpan="3">No AAS to show.</td>
      </tr>
    );
  };
      ...
      <nav className="navbar navbar-light bg-light">
        <form className="form-inline" onSubmit={handleSubmit}>
          <div className="form-group">
            <label className="sr-only" htmlFor="aasId">
              UUID
            </label>
            <input
        ...
        <Link className="btn btn-warning float-right" to="/aas/new">
          Create AAS
        </Link>
      </nav>
